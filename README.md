# 🗂️ 20 Proje Koleksiyonu

## 📌 Genel Bakış  
Bu repo, **20 farklı mini proje** içermektedir. Projelerde, farklı teknolojileri (ADO.NET, Entity Framework, Dapper, Web API, RapidAPI, MongoDB, PostgreSQL, JWT, Kaggle Dataset, Ollama AI vb.) kullanarak geliştirdim.  

Amacım, **farklı veri tabanları, ORM araçları, API tüketimi ve modern .NET teknolojilerini** gerçek dünya senaryoları ile geliştirmek.  

---

## 📊 Proje Detayları

| #  | Proje Adı | Açıklama | Teknolojiler | Tür |
|----|-----------|----------|--------------|-----|
| 1  | AdoNetCustomerProject | SQL Server ile müşteri ve şehir verilerini dinamik olarak yönetir | .NET Framework, ADO.NET, SQL Server | 🖥️ WinForms |
| 2  | EntityFrameworkProductManagementProject | DbFirst yaklaşımı ile ürün ve kategori yönetir | Entity Framework (DbFirst), SQL Server | 🖥️ WinForms |
| 3  | EntityFrameworkStatisticsProject | Linq ile verileri istatistiksel olarak analiz eder | Entity Framework, SQL Server | 🖥️ WinForms |
| 4  | EntityFrameworkCodeFirstMovieProject | Code First yaklaşımıyla film ve dizi listesi oluşturur | EF Code First, SQL Server | 🖥️ WinForms |
| 5  | DapperNorthwindProject | Northwind veri tabanı üzerinde Dapper ile CRUD işlemleri gerçekleştirir | Dapper, SQL Server | 🖥️ WinForms |
| 6  | APIWeatherProject | Şehir ve hava durumu verilerini sağlar | ASP.NET Core Web API, EF Core | 🌐 API |
| 7  | APIWeatherConsumeProject | API verilerini C# Console ile tüketir | .NET 6, API Consume | 📟 Console |
| 8  | RapidAPICurrencyProject | Gerçek zamanlı döviz kurlarını RapidAPI üzerinden çeker | RapidAPI, JSON | 🖥️ WinForms |
| 9  | MongoDbOrderProject | MongoDB tabanlı sipariş kayıt ve yönetimini sağlar | MongoDB, .NET Framework | 🖥️ WinForms |
| 10 | PostgreSQLToDoListProject | PostgreSQL ile To Do list yönetimini sağlar | PostgreSQL | 🖥️ WinForms |
| 11 | SQLTriggerOrderProject | Parametreli ve parametresiz SQL tetikleyicilerle sipariş yönetir | EF, SQL Triggers | 📟 Console |
| 12 | JSONWebTokenProject | JWT ile oturum doğrulama ve token tabanlı kimlik yönetir | JWT, SQL Server | 🖥️ WinForms |
| 13 | RapidAPIWeatherProject | RapidAPI üzerinden çekilen hava durumu bilgileriyle hava kartı gösterir | RapidAPI, JSON | 🖥️ WinForms |
| 14 | SQLAllRelationsProject | SQL ilişkileri (1:1, 1:N, N:N) ile Windows Forms yönetimini sağlar | ADO.NET, SQL Server | 🖥️ WinForms |
| 15 | RapidAPIGasPriceSimulationProject | Güncel akaryakıt fiyatlarıyla mini simülasyon yapar | RapidAPI, JSON | 🖥️ WinForms |
| 16 | MailRegisterProject | Aktivasyon kodu ile e-posta doğrulama yapar | EF, MailKit, SQL Server | 🖥️ WinForms |
| 17 | KaggleDatasetProject | Kaggle veri setleriyle veri analizi ve örnek uygulamalar yapar | ADO.NET, SQL Server | 📟 Console |
| 18 | SuperStoreDatasetStatsProject | Dataset ile Dashboard oluşturur | ADO.NET, Chart Controls | 🖥️ WinForms |
| 19 | RestaurantTableCurrentStatusProject | Restoran masalarının anlık doluluk ve boşluk durumlarını gösterir | EF, SQL Server | 🖥️ WinForms |
| 20 | OllamaAIChatProject | Ollama AI ile konsol sohbeti yapar | .NET 9, OllamaSharp | 📟 Console |
...
### 🔹 [4. EntityFrameworkCodeFirstMovieProject](./4-EntityFrameworkCodeFirstMovieProject)  
